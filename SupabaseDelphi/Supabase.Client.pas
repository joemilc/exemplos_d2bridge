...
type
  ...
  ISupabaseRequest = interface
    ...
    function Filter(const AFilter: string): ISupabaseRequest;
    function Eq(const AField, AValue: string): ISupabaseRequest;
    function Limit(ALimit: Integer): ISupabaseRequest;
    function Offset(AOffset: Integer): ISupabaseRequest;
    function Order(const AField: string; Ascending: Boolean = True): ISupabaseRequest;
...
function TSupabaseClient.Filter(const AFilter: string): ISupabaseRequest;
begin
  if FFilter <> '' then
    FFilter := FFilter + '&';
  FFilter := FFilter + AFilter;
  Result := Self;
end;
...
function TSupabaseClient.Offset(AOffset: Integer): ISupabaseRequest;
begin
  FOffset := Format('offset=%d', [AOffset]);
  Result := Self;
end;

function TSupabaseClient.Order(const AField: string; Ascending: Boolean): ISupabaseRequest;
var
  Dir: string;
begin
  if Ascending then Dir := 'asc' else Dir := 'desc';
  if FOrder <> '' then
    FOrder := FOrder + '&';
  FOrder := FOrder + Format('order=%s.%s', [AField, Dir]);
  Result := Self;
end;
...
begin
  ...
  case FOperation of
    ...
    begin
      LStream := TStringStream.Create(FBody, TEncoding.UTF8);
      try
        LResp := FHttp.Patch(LUrl, LStream);
      finally
        LStream.Free;
      end;
    end;
